# ══════════════════════════════════════════════════════════════════
# app.py — Mshauri wa Malipo ya Kigeni | Tanzania  (v3 – Fixed)
# ══════════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────────────────────────────
# LANGUAGE — resolve BEFORE any translate() call
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS = {
    'en': {
        ...
        'compare_title':  '🌍 Live Currency Comparison',
        'header_title':   '💱 Foreign Payment Advisor',
        'header_sub':     'Helps you decide when to pay in USD, EUR, or CNY',
        'preset_label':   'Quick presets:',
        'date_toggle':    'Set my own payment date',
        'best_day_title': '📅 Best Day to Pay — Historical Analysis',
        'ushauri_label':  "Today's Advice",
        'hesabu_label':   'Your Payment Calculation',
    },
    'sw': {
        'sidebar_title':  '⚙️ Mipangilio',
        'purpose_label':  'Unataka kulipa nini?',
        'currency_label': 'Sarafu ya malipo',
        'timeframe_label':'Angalia mwenendo wa muda gani?',
        'refresh_button': '🔄 Onyesha upya',
        'payment_title':  '🧮 Kiasi cha Malipo',
        'download_csv':   '⤓ Pakua utabiri (CSV)',
        ...
        'header_sub':     'Inakusaidia kujua wakati mzuri wa kulipa kwa USD, EUR, au CNY',
        'preset_label':   'Chaguo la haraka:',
        'date_toggle':    'Weka tarehe yangu mwenyewe',
        'best_day_title': '📅 Siku Bora ya Kulipa — Uchambuzi wa Kihistoria',
        'ushauri_label':  'Ushauri wa Leo',
        'hesabu_label':   'Hesabu ya Fedha Yako',
    },
    'zh': {
        'sidebar_title':  '⚙️ 设置',
        ...
        'preset_label':   '快速预设:',
        'date_toggle':    '自定义支付日期',
        'best_day_title': '📅 最佳支付日 — 历史分析',
        'ushauri_label':  '今日建议',
        'hesabu_label':   '您的付款计算',
    },
}

# FIX 1: Resolve language from session_state BEFORE any translate() call
if 'lang' not in st.session_state:
    st.session_state['lang'] = 'sw'

def T(key):
    """Safe translate — always reads from session_state."""
    lang = st.session_state.get('lang', 'sw')
...
.hero-banner {
    ...
    position: relative;
    overflow: hidden;
    box-shadow: 0 4px 6px rgba(79,70,229,0.12), 0 20px 60px rgba(79,70,229,0.25),
                0 40px 100px rgba(79,70,229,0.12), inset 0 1px 0 rgba(255,255,255,0.25);
    display: flex;
...
.hero-title {
    ...
    color:#fff; margin:0 0 0.3rem 0; letter-spacing:-0.8px;
}
.hero-sub { font-size:0.92rem; color:rgba(255,255,255,0.82); margin:0; }
.hero-right { position:relative; z-index:2; }
.hero-rate-pill {
    background:rgba(255,255,255,0.15); backdrop-filter:blur(12px);
    border:1px solid rgba(255,255,255,0.30); border-radius:16px;
    padding:0.9rem 1.4rem; display:inline-block;
    box-shadow:inset 0 1px 0 rgba(255,255,255,0.2), 0 4px 16px rgba(0,0,0,0.15);
}
.hrp-label { font-size:0.68rem; color:rgba(255,255,255,0.75); text-transform:uppercase; letter-spacing:1.2px; font-weight:600; }
.hrp-val   { font-family:'JetBrains Mono',monospace; font-size:1.7rem; font-weight:600; color:#fff; line-height:1.15; }
.hrp-change { font-size:0.78rem; color:rgba(255,255,255,0.88); }

/* ── GLASS CARDS ── */
.glass-card {
    background:linear-gradient(145deg, rgba(255,255,255,0.92) 0%, rgba(255,255,255,0.75) 100%);
    backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px);
    border-radius:20px; border:1px solid rgba(255,255,255,0.80);
...
.gc-label { font-size:0.70rem; font-weight:600; text-transform:uppercase; letter-spacing:1px; color:#6b7280; margin-bottom:0.4rem; }
.gc-value { font-family:'JetBrains Mono',monospace; font-size:1.5rem; font-weight:600; color:#111827; line-height:1.2; }
.gc-note  { font-size:0.75rem; color:#9ca3af; margin-top:0.25rem; }
.gc-icon  { position:absolute; top:1rem; right:1.2rem; font-size:1.6rem; opacity:0.15; }

/* ── RECOMMENDATION CARDS ── */
.rec-3d {
    border-radius:22px; padding:2rem 2.2rem; margin-bottom:1rem;
    position:relative; overflow:hidden;
...
.rec-3d::after {
    ...
    opacity:0.18; background:rgba(255,255,255,0.6);
}
.rec-go {
    background:linear-gradient(135deg,#d1fae5 0%,#a7f3d0 30%,#6ee7b7 60%,#34d399 100%);
    border:1.5px solid rgba(16,185,129,0.40);
    box-shadow:inset 0 1px 0 rgba(255,255,255,0.8),0 20px 60px rgba(16,185,129,0.25),0 8px 24px rgba(16,185,129,0.15),0 2px 4px rgba(0,0,0,0.06);
    color:#064e3b;
}
.rec-wait {
    background:linear-gradient(135deg,#fef3c7 0%,#fde68a 30%,#fcd34d 60%,#fbbf24 100%);
    border:1.5px solid rgba(245,158,11,0.40);
    box-shadow:inset 0 1px 0 rgba(255,255,255,0.8),0 20px 60px rgba(245,158,11,0.22),0 8px 24px rgba(245,158,11,0.15),0 2px 4px rgba(0,0,0,0.06);
    color:#78350f;
}
.rec-watch {
    background:linear-gradient(135deg,#dbeafe 0%,#bfdbfe 30%,#93c5fd 60%,#60a5fa 100%);
    border:1.5px solid rgba(59,130,246,0.35);
    box-shadow:inset 0 1px 0 rgba(255,255,255,0.8),0 20px 60px rgba(59,130,246,0.22),0 8px 24px rgba(59,130,246,0.15),0 2px 4px rgba(0,0,0,0.06);
    color:#1e3a8a;
}
.rec-3d-action  { font-family:'Syne',sans-serif; font-size:1.7rem; font-weight:800; display:flex; align-items:center; gap:10px; margin-bottom:0.7rem; letter-spacing:-0.5px; position:relative; z-index:2; }
.rec-3d-headline { font-size:0.95rem; line-height:1.65; margin-bottom:0.6rem; position:relative; z-index:2; font-weight:500; }
.rec-3d-purpose  { font-size:0.88rem; font-style:italic; opacity:0.82; position:relative; z-index:2; }

/* ── PAYMENT BOXES ── */
.pay-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin:1rem 0; }
.pay-3d {
...
.pay-3d:hover { transform:translateY(-3px) scale(1.02); }
.pay-today       { background:linear-gradient(135deg,#e0e7ff 0%,#c7d2fe 50%,#a5b4fc 100%); border:1px solid rgba(99,102,241,0.30); box-shadow:0 8px 32px rgba(99,102,241,0.18),0 2px 8px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.7); color:#1e1b4b; }
.pay-future-okoa  { background:linear-gradient(135deg,#d1fae5 0%,#a7f3d0 50%,#6ee7b7 100%); border:1px solid rgba(16,185,129,0.30); box-shadow:0 8px 32px rgba(16,185,129,0.18),0 2px 8px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.7); color:#064e3b; }
.pay-future-hasara { background:linear-gradient(135deg,#fee2e2 0%,#fecaca 50%,#fca5a5 100%); border:1px solid rgba(239,68,68,0.30); box-shadow:0 8px 32px rgba(239,68,68,0.15),0 2px 8px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.7); color:#7f1d1d; }
.pay-future-stable { background:linear-gradient(135deg,#dbeafe 0%,#bfdbfe 50%,#93c5fd 100%); border:1px solid rgba(59,130,246,0.30); box-shadow:0 8px 32px rgba(59,130,246,0.15),0 2px 8px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.7); color:#1e3a8a; }
...
.savings-banner {
    ...
    flex-wrap:wrap; gap:10px; margin-top:0.5rem;
    animation:fadeSlideIn 0.7s ease both;
}
.sb-okoa   { background:linear-gradient(135deg,#ecfdf5,#d1fae5); border:1px solid rgba(16,185,129,0.35); box-shadow:0 4px 20px rgba(16,185,129,0.12); color:#064e3b; }
.sb-hasara { background:linear-gradient(135deg,#fff1f2,#fee2e2); border:1px solid rgba(239,68,68,0.30); box-shadow:0 4px 20px rgba(239,68,68,0.10); color:#7f1d1d; }
.sb-stable { background:linear-gradient(135deg,#eff6ff,#dbeafe); border:1px solid rgba(59,130,246,0.28); box-shadow:0 4px 20px rgba(59,130,246,0.10); color:#1e3a8a; }
...
.insight-3d::before {
    ...
    width:100px; height:100px; border-radius:50%; background:rgba(255,255,255,0.25);
}
.ins-purple { background:linear-gradient(135deg,#ede9fe 0%,#ddd6fe 50%,#c4b5fd 100%); border:1px solid rgba(139,92,246,0.28); box-shadow:0 8px 32px rgba(139,92,246,0.18),inset 0 1px 0 rgba(255,255,255,0.6); color:#2e1065; }
.ins-indigo { background:linear-gradient(135deg,#e0e7ff 0%,#c7d2fe 50%,#a5b4fc 100%); border:1px solid rgba(99,102,241,0.28); box-shadow:0 8px 32px rgba(99,102,241,0.18),inset 0 1px 0 rgba(255,255,255,0.6); color:#1e1b4b; }
.ins-teal   { background:linear-gradient(135deg,#ccfbf1 0%,#99f6e4 50%,#5eead4 100%); border:1px solid rgba(20,184,166,0.28); box-shadow:0 8px 32px rgba(20,184,166,0.18),inset 0 1px 0 rgba(255,255,255,0.6); color:#042f2e; }
...
.data-warn-3d {
    ...
    border-radius:14px; padding:0.8rem 1.1rem; font-size:0.83rem;
    color:#78350f; margin-bottom:1.2rem; box-shadow:0 4px 16px rgba(245,158,11,0.12);
}

/* ── EYEBROW ── */
.eyebrow {
    font-family:'Syne',sans-serif; font-size:0.68rem; font-weight:700;
    text-transform:uppercase; letter-spacing:1.4px; color:#6366f1; margin-bottom:0.55rem;
}

/* ── MODEL CARDS ── */
.model-3d {
    background:linear-gradient(145deg,rgba(255,255,255,0.92),rgba(240,245,255,0.85));
    border:1px solid rgba(255,255,255,0.80); border-radius:18px;
    padding:1.1rem 1.4rem; margin-bottom:0.8rem;
    display:flex; justify-content:space-between; align-items:center;
    gap:12px; flex-wrap:wrap;
    box-shadow:0 4px 20px rgba(99,102,241,0.08),inset 0 1px 0 rgba(255,255,255,0.9),0 12px 32px rgba(0,0,0,0.06);
    transition:transform 0.22s cubic-bezier(.34,1.56,.64,1), box-shadow 0.22s;
    animation:fadeSlideIn 0.5s ease both;
}
.model-3d:hover { transform:translateY(-3px); box-shadow:0 8px 32px rgba(99,102,241,0.15),inset 0 1px 0 rgba(255,255,255,0.9); }
...
.explainer-3d {
    ...
    border:1px solid rgba(99,102,241,0.20); border-left:4px solid #6366f1;
    border-radius:14px; padding:1.1rem 1.4rem; font-size:0.87rem;
    line-height:1.75; color:#2d2b5e; box-shadow:0 4px 16px rgba(99,102,241,0.08);
}

/* ── CURRENCY CARDS ── */
.cur-cards { display:grid; grid-template-columns:repeat(auto-fit, minmax(200px,1fr)); gap:1rem; margin-bottom:1rem; }
.cur-card {
    background:linear-gradient(145deg,rgba(255,255,255,0.92),rgba(245,247,255,0.85));
    border:1px solid rgba(255,255,255,0.78); border-radius:16px;
    padding:1rem 1.3rem; margin-bottom:0;
    box-shadow:0 4px 16px rgba(99,102,241,0.07),inset 0 1px 0 rgba(255,255,255,0.9);
...
.cur-card.best-cur {
    ...
    box-shadow:0 8px 28px rgba(16,185,129,0.15),inset 0 1px 0 rgba(255,255,255,0.9);
}
...
/* ── URGENCY CHIP ── */
.urg-chip { display:inline-flex; align-items:center; gap:6px; border-radius:30px; padding:0.38rem 1rem; font-size:0.78rem; font-weight:600; letter-spacing:0.2px; margin-bottom:1rem; }
...
.sb-title {
    font-family:'Syne',sans-serif; font-size:0.68rem; font-weight:700;
    text-transform:uppercase; letter-spacing:1.2px; color:#6366f1;
    margin:1.1rem 0 0.45rem 0; padding-bottom:0.3rem;
    border-bottom:1px solid rgba(99,102,241,0.20);
}
.preset-lbl { font-size:0.70rem; font-weight:600; color:#9ca3af; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:0.4rem; }

/* ── TABS ── */
.stTabs [data-baseweb="tab-list"] {
    background:rgba(255,255,255,0.70); backdrop-filter:blur(10px);
    border-radius:14px; padding:5px; gap:4px;
    border:1px solid rgba(99,102,241,0.15); box-shadow:0 4px 16px rgba(99,102,241,0.08);
}
.stTabs [data-baseweb="tab"] {
    border-radius:10px; font-weight:500; font-size:0.88rem;
    padding:0.5rem 1.2rem; color:#6b7280; border:none; background:transparent; transition:all 0.2s;
}
.stTabs [aria-selected="true"] {
    background:linear-gradient(135deg,#6366f1,#7c3aed) !important;
    color:#fff !important; box-shadow:0 4px 14px rgba(99,102,241,0.35);
}
...
.chart-wrap {
    ...
    border-radius:20px; border:1px solid rgba(255,255,255,0.80);
    padding:1rem; box-shadow:0 8px 32px rgba(99,102,241,0.10),inset 0 1px 0 rgba(255,255,255,0.9);
    margin-bottom:1rem;
}
...
.shimmer-line {
    ...
    background-size:200% 100%; animation:shimmer 1.8s infinite; margin-bottom:1rem;
}
...
PRESETS = {
    ...
    "bidhaa":    [("Ndogo 100", 100.0), ("Kati 500",  500.0), ("Kubwa 2k", 2000.0)],
...
def load_models():
    ...
    for cur in ['usd','eur','cny']:
        models[cur] = {'arima':None,'prophet':None,'xgboost':None,'lstm':None,'scaler':None}
        try:
...
def pred_arima(model, steps):
    try:    return model.forecast(steps=steps).values
    except: return None

def pred_prophet(model, steps):
    try:
        future = model.make_future_dataframe(periods=steps)
        return model.predict(future)['yhat'].tail(steps).values
    except: return None

def pred_xgboost(model, df, steps):
    try:
        from datetime import datetime
        preds = []; last_data = df['mean'].values.copy(); base_date = datetime.now()
        for i in range(steps):
            ...
            feat = np.array([[l1,l3,l7,l14,l30,ma7,ma30,std7,fd.month,fd.weekday(),(fd.month-1)//3+1]])
            p = model.predict(feat)[0]; preds.append(p); last_data = np.append(last_data, p)
        return np.array(preds)
    except: return None

def pred_lstm(model, scaler, df, steps, seq=60):
    try:
        if len(df) < seq: return None
        scaled = scaler.transform(df[['mean']]); last_seq = scaled[-seq:].copy(); preds = []
        for _ in range(steps):
            X = last_seq[-seq:].reshape(1,seq,1)
            p = model.predict(X, verbose=0)[0][0]
            preds.append(p); last_seq = np.append(last_seq, [[p]], axis=0)
...
def get_ensemble(models_dict, df, cur_key, steps):
    ...
    return np.mean(list(forecasts.values()), axis=0), forecasts

def get_confidence_band(all_forecasts, ensemble):
    if len(all_forecasts) < 2:
        s = ensemble * 0.005; return ensemble-s, ensemble+s
...
def best_day_analysis(df):
    ...
    return {
        'best_weekday':  day_sw.get(wd.idxmin(), wd.idxmin()),
        'best_wom':      wom_lbl.get(int(wom.idxmin()), f"Wiki {wom.idxmin()}"),
...
def get_advice(current_rate, ensemble, payment_amount, cur_name, purpose_key, steps):
    ...
    if change_pct > 1.0:
        ...
        saving_direction="hasara"; saving_amount=abs(tzs_now-tzs_later)
    elif change_pct < -1.0:
        ...
        saving_direction="okoa"; saving_amount=abs(tzs_now-tzs_later)
    else:
    ...
    if abs(change_pct)>=2.0: urg_level="Juu";   urg_class="urg-high"; urg_note="Mabadiliko makubwa — chukua hatua haraka."
    elif abs(change_pct)>=1.0: urg_level="Kati"; urg_class="urg-mid";  urg_note="Mabadiliko wastani — angalia kabla ya kulipa."
    else:                       urg_level="Ndogo"; urg_class="urg-low"; urg_note="Mabadiliko madogo — hatari ni ndogo."

    pm = {
        "streaming":{"LIPA SASA":"Lipia subscription sasa — bei itapanda. 🎬","SUBIRI":f"Ukisubiri siku {steps}, utaokoa TZS. 🎬","ANGALIA":"Bei iko stable — kulipa wakati wowote. 🎬"},
        "bidhaa":   {"LIPA SASA":f"Lipia invoice ya {cur_name} {payment_amount:,.0f} sasa. 📦","SUBIRI":"Subiri — bei itashuka na utaokoa. 📦","ANGALIA":"Bei iko stable. Kulipa wakati wowote. 📦"},
        "safari":   {"LIPA SASA":f"Nunua {cur_name} kwa safari sasa — bei itapanda. ✈️","SUBIRI":"Subiri — bei itashuka. ✈️","ANGALIA":"Bei iko stable. Kulipa wakati wowote. ✈️"},
        "kutuma":   {"LIPA SASA":"Tuma pesa sasa! Bei itapanda. 📤","SUBIRI":f"Subiri siku {steps} — bei itashuka. 📤","ANGALIA":"Bei iko stable. Kutuma wakati wowote. 📤"},
        "masomo":   {"LIPA SASA":"Lipia ada sasa — bei itapanda. 🎓","SUBIRI":"Kama tarehe inaruhusu, subiri. 🎓","ANGALIA":"Bei iko stable — panga kulingana na tarehe. 🎓"},
        "afya":     {"LIPA SASA":"Lipia sasa — bei itapanda. Afya ni muhimu! 💊","SUBIRI":"Kama hali inaruhusu, subiri. 💊","ANGALIA":"Bei iko stable. Kulipa wakati wowote. 💊"},
        "nyingine": {"LIPA SASA":"Fanya malipo sasa — bei itapanda.","SUBIRI":"Subiri — bei itashuka.","ANGALIA":"Bei iko stable."},
    }
    return {
        "action":action,"rec_style":rec_style,"icon":icon,
        "headline":headline,"purpose_text":pm.get(purpose_key,pm["nyingine"])[action],
        "change_pct":change_pct,"tzs_now":tzs_now,"tzs_later":tzs_later,
...
# ─────────────────────────────────────────────────────────────────
# SIDEBAR  — FIX 2: language set into session_state here
# ─────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("""
    ...
    # FIX: language selector writes to session_state; T() reads from session_state
    lang_idx = {"sw":0,"en":1,"zh":2}.get(st.session_state.get('lang','sw'), 0)
    lang_choice = st.selectbox("Language / Lugha / 语言",
                               ["Kiswahili (sw)","English (en)","中文 (zh)"],
                               index=lang_idx)
    st.session_state['lang'] = lang_choice[-3:-1]   # extracts 'sw','en','zh'

    st.markdown('<div class="sb-title">🎯 Lengo la Malipo</div>', unsafe_allow_html=True)
    purpose_label = st.selectbox(T('purpose_label'), list(PURPOSES.keys()), label_visibility="collapsed")
    purpose_key   = PURPOSES[purpose_label]

    st.markdown('<div class="sb-title">💱 Sarafu &amp; Muda</div>', unsafe_allow_html=True)
    currency_label = st.selectbox(T('currency_label'), list(CURRENCIES.keys()), label_visibility="collapsed")
    cur_display, cur_key = CURRENCIES[currency_label]

    timeframe_label = st.selectbox(T('timeframe_label'), list(TIMEFRAMES.keys()))
    steps = TIMEFRAMES[timeframe_label]

    st.markdown("**— au —** " + T('date_toggle') + ":")
    use_date = st.checkbox(T('date_toggle'))
    if use_date:
    ...
    st.markdown(f'<div class="sb-title">{T("payment_title")}</div>', unsafe_allow_html=True)
    presets = PRESETS.get(purpose_key, PRESETS["nyingine"])
    st.markdown(f'<p class="preset-lbl">{T("preset_label")}</p>', unsafe_allow_html=True)
    pc = st.columns(3)
    preset_value = None
    ...
    st.markdown(f"""<p style='font-size:0.73rem;color:#9ca3af;line-height:1.6;margin-top:0.8rem;'>
...
if ensemble is None:
    st.error("❌ Modeli hazikuweza kutoa utabiri. Angalia model files."); st.stop()
...
# ─────────────────────────────────────────────────────────────────
# HERO BANNER  — FIX 3: self-contained div, no columns split
# ─────────────────────────────────────────────────────────────────
...
<div class="hero-banner">
    <div class="hero-left">
        <div class="hero-title">{T('header_title')}</div>
        <p class="hero-sub">{T('header_sub')}</p>
    </div>
    <div class="hero-right">
        <div class="hero-rate-pill">
            <div class="hrp-label">Bei ya Sasa · {cur_display}/TZS</div>
            <div class="hrp-val">TZS {current_rate:,.2f}</div>
            <div class="hrp-change">{arrow} {abs(rate_delta):.2f} ({abs(rate_delta_p):.2f}%) tangu jana</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────
# TABS  — FIX 4: tab labels use T() AFTER session_state is set
# ─────────────────────────────────────────────────────────────────
tab1, tab2, tab3 = st.tabs([T('tab_advice'), T('tab_trend'), T('tab_analysis')])
...
with tab1:
    ...
    with c1:
        st.markdown(f"""<div class="glass-card g-indigo">
            ...
            <div class="gc-icon">📊</div></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="glass-card g-emerald">
            ...
            <div class="gc-icon">🏦</div></div>""", unsafe_allow_html=True)
    with c3:
        st.markdown(f"""<div class="glass-card g-violet">
            ...
            <div class="gc-icon">💰</div></div>""", unsafe_allow_html=True)
    with c4:
        ug = {"urg-high":"g-rose","urg-mid":"g-amber","urg-low":"g-emerald"}
        st.markdown(f"""<div class="glass-card {ug.get(advice['urgency_class'],'g-sky')}">
            ...
            <div class="gc-icon">⚠️</div></div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    col_rec, col_pay = st.columns([1.1, 1], gap="large")
    with col_rec:
        st.markdown(f'<div class="eyebrow">{T("ushauri_label")}</div>', unsafe_allow_html=True)
        st.markdown(f"""
    ...
    with col_pay:
        st.markdown(f'<div class="eyebrow">{T("hesabu_label")}</div>', unsafe_allow_html=True)
        fc = ("pay-future-okoa"   if advice['saving_direction']=='okoa'
        ...
        if advice['saving_direction']=='okoa':
            st.markdown(f"""<div class="savings-banner sb-okoa">
                <div><div class="sb-label">💡 Ukifuata ushauri, utaokoa:</div>
                     <div class="sb-sub">Tofauti kati ya leo na baada ya siku {steps}</div></div>
                <div class="sb-amount">TZS {advice['saving_amount']:,.0f}</div></div>""", unsafe_allow_html=True)
        elif advice['saving_direction']=='hasara':
            st.markdown(f"""<div class="savings-banner sb-hasara">
            ...
            st.markdown(f"""<div class="savings-banner sb-stable">
                <div><div class="sb-label">📊 Tofauti ya kulipa leo vs siku {steps}:</div>
                     <div class="sb-sub">Bei iko stable — hakuna haraka</div></div>
                <div class="sb-amount">TZS {abs(advice['tzs_now']-advice['tzs_later']):,.0f}</div></div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f'<div class="eyebrow">{T("best_day_title")}</div>', unsafe_allow_html=True)
    ic1, ic2, ic3 = st.columns(3)
    with ic1:
        st.markdown(f"""<div class="insight-3d ins-purple">
            <div class="ins-label">Siku Bora ya Wiki</div>
            <div class="ins-value">🗓 {bda['best_weekday']}</div>
            <div class="ins-note">Bei ya wastani iko chini zaidi siku hii</div></div>""", unsafe_allow_html=True)
    with ic2:
        st.markdown(f"""<div class="insight-3d ins-indigo">
            <div class="ins-label">Wiki Bora ya Mwezi</div>
            <div class="ins-value">📆 {bda['best_wom']}</div>
            <div class="ins-note">Bei iko chini zaidi katika wiki hii</div></div>""", unsafe_allow_html=True)
    with ic3:
        st.markdown(f"""<div class="insight-3d ins-teal">
            <div class="ins-label">Akiba ya Wastani kwa Wiki</div>
            <div class="ins-value">💰 TZS {bda['avg_saving_wd']:,.0f}</div>
            <div class="ins-note">Tofauti kati ya siku bora na mbaya zaidi</div></div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    fig.add_trace(go.Scatter(
        x=future_dates+future_dates[::-1], y=list(upper_band)+list(lower_band[::-1]),
        fill='toself', fillcolor='rgba(99,102,241,0.10)',
        line=dict(color='rgba(255,255,255,0)'), showlegend=True,
        name='Upeo wa Utabiri (±)', hoverinfo='skip'))

    fig.add_trace(go.Scatter(
        x=hist['date'], y=hist['mean'], name='Bei ya Kihistoria',
        line=dict(color='#4f46e5', width=2.5), mode='lines',
        hovertemplate='%{x|%d/%m/%Y}<br>TZS %{y:,.2f}<extra></extra>'))

    if 'MA_30' in hist.columns:
        fig.add_trace(go.Scatter(
            x=hist['date'], y=hist['MA_30'], name='Wastani wa Siku 30',
            line=dict(color='#f59e0b', width=1.8, dash='dot'), mode='lines',
            hovertemplate='MA30: TZS %{y:,.2f}<extra></extra>'))

    fig.add_trace(go.Scatter(
        x=future_dates, y=ensemble, name=f'Utabiri (Siku {steps})',
        line=dict(color='#10b981', width=3), mode='lines+markers',
        marker=dict(size=6, color='#10b981', line=dict(color='white', width=2)),
    ...
    fig.update_layout(
        ...
        legend=dict(orientation='h',yanchor='bottom',y=1.02,
                    bgcolor='rgba(255,255,255,0.85)',bordercolor='rgba(99,102,241,0.20)',borderwidth=1),
        xaxis=dict(title='Tarehe',showgrid=True,gridcolor='rgba(99,102,241,0.08)',zeroline=False,tickfont=dict(size=11)),
        yaxis=dict(title=f'TZS kwa 1 {cur_display}',showgrid=True,gridcolor='rgba(99,102,241,0.08)',zeroline=False,tickfont=dict(size=11)),
        hovermode='x unified', plot_bgcolor='rgba(255,255,255,0.95)', paper_bgcolor='rgba(0,0,0,0)',
    ...
    for i,(d,v) in enumerate(zip(future_dates, ensemble)):
        change = v - current_rate; chg_p = change/current_rate*100
    ...
    # FIX 5: use_container_width=True  (not width='stretch')
...
with tab3:
    ...
    with col_l:
        ...
        model_info = {
            'XGBoost':{'mae':{'usd':3.63,'eur':10.79,'cny':1.49},'mape':{'usd':0.14,'eur':0.36,'cny':0.40},'desc':'Inajifunza patterns kutoka vipengele vingi — mwelekeo wa wiki na mwezi'},
            'LSTM':   {'mae':{'usd':5.33,'eur':22.69,'cny':1.12},'mape':{'usd':0.21,'eur':0.76,'cny':0.30},'desc':'Ina "kumbukumbu" ya muda mrefu — inajifunza mifuatano ya bei'},
        }
        for mname, mdata in model_info.items():
            mae = mdata['mae'].get(cur_key,0); mape = mdata['mape'].get(cur_key,0)
            st.markdown(f"""<div class="model-3d">
                <div><div class="mn">{mname}</div><div class="md">{mdata['desc']}</div></div>
                <div class="mr"><div class="mel">Kosa la wastani</div>
                <div class="mev">TZS {mae:.2f} ({mape:.2f}%)</div></div></div>""", unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="eyebrow">ℹ️ Jinsi Inavyofanya Kazi</div>', unsafe_allow_html=True)
        st.markdown("""<div class="explainer-3d">
            ...
            unatoa ushauri. Upeo wa utabiri unaonyesha pengo la kutokuwa na uhakika.
            Ushauri ni kwa mtu anayetaka <strong>KULIPA</strong> kwa sarafu ya kigeni.
        </div>""", unsafe_allow_html=True)

    with col_r:
        st.markdown(f'<div class="eyebrow">{T("compare_title")}</div>', unsafe_allow_html=True)
        comparison_rows = []
        for label,(display,key) in CURRENCIES.items():
            od = all_data[display]; cv = float(od['mean'].iloc[-1]); pv = float(od['mean'].iloc[-2])
            chg = cv-pv; chgp = chg/pv*100
        ...
        html = '<div class="cur-cards">'
        for row in comparison_rows:
            is_best = row['display']==best['display']
            badge = '<div style="font-size:0.72rem;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:0.5rem">✅ Nafuu Zaidi</div>' if is_best else ''
            html += f'<div class="cur-card{"  best-cur" if is_best else ""}">'
            html += badge
            html += f'<div class="cur-row"><span class="cur-lbl">Sarafu</span><span class="cur-val">{row["display"]}</span></div>'
            html += f'<div class="cur-row"><span class="cur-lbl">Bei (TZS)</span><span class="cur-val">{row["fmt"]}</span></div>'
            html += f'<div class="cur-row"><span class="cur-lbl">Mabadiliko</span><span class="cur-val">{row["chg"]}</span></div>'
            html += '</div>'
        html += '</div>'
        ...
        if len(all_forecasts) > 1:
            ...
            for mname,preds in all_forecasts.items():
                fig2.add_trace(go.Scatter(x=future_dates,y=preds,name=mname,
                    line=dict(color=cmap.get(mname,'#888'),width=2),mode='lines',
                    hovertemplate=f'{mname}: TZS %{{y:,.2f}}<extra></extra>'))
            fig2.add_trace(go.Scatter(x=future_dates,y=ensemble,name='Ensemble',
                line=dict(color='#f59e0b',width=3,dash='dash'),
                hovertemplate='Ensemble: TZS %{y:,.2f}<extra></extra>'))
            fig2.update_layout(
                height=270, margin=dict(l=0,r=0,t=20,b=0),
                legend=dict(orientation='h',yanchor='bottom',y=1.02,bgcolor='rgba(255,255,255,0.85)'),
